...
},start:function(a){this._seconds=a;
this.go()
},go:function(){if(!this._isRunning){var a=this;
this._isRunning=true;
this._id=setInterval(function(){a._tick()
},this._interval)
}},_tick:function(){if(this._seconds<=this._tickCounter){this._callback(this._id);
this.reset()
}else{this._tickCounter++
}},stop:function(){this._isRunning=false;
...
if(this._reminder>-1&&a>(this._reminder*1000)){this.showRating();
this._reminder=-1
}},handleMediaStart:function(b){var a=b.data;
if(a.baseClip&&a.baseClip.isAd){this._shouldBeDisplayed=true;
this.hideRating()
...
},handleMediaPause:function(a){if(this.ratingsTimer.isRunning()){this.ratingsTimer.stop()
}},handleMediaUnpause:function(a){if(!this.ratingsTimer.isRunning()){this.ratingsTimer.go()
...
}},showRating:function(){var b=this._namingProperties[this.imageManager.isHD()?"HD":"SD"][this._isFirstClip?0:1];
var a=this.nameMap.getImageName(this.loadVars.getScheme(),b);
this.imageManager.setImageSrc(this.loadVars.getPath()+a);
if(!this.ratingsTimer.isRunning()){this.ratingsTimer.start(this.loadVars.getDelay())
}},hideRating:function(){this.imageManager.hide()
...
},setImageSrc:function(a){if(this._srcLoaded){if(this._currentSrc==a){return
}}this.hide();
this._srcLoaded=false;
