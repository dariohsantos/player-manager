...
{
    //if cross-domain the sender will be null
    if (sender)
    {
    ...
    else //we'll just go with what was set on the last tpLastSMFBridge.  This will not be dependable if two silverlight instances load up at the exact same time
    {
        var swfId = tpLastSMFBridge.swfId;
        var controllerId = tpLastSMFBridge.controllerId;
    }
...
function tpExternal_SMF(swfId, controllerId)
{
    ...
    this.startPlay = function()
    {
        ...
        this.SMF.SetControlsVisible(!this.firstLoad && this.useNativeControls);
	...
	this.report = function()
	{
        ...
        if (this.SMFDiv.style.zIndex == "100")
        {
            this.SMFDiv.style.zIndex = "101";
        }
	...
	this.cleanup = function()
	{
		if (this.SMFDiv)
		{
			...
			var the_div = document.getElementById(this.SMFDIV);
            var the_parent = the_div.parentNode;
            the_parent.removeChild(the_div);
		}
	};

	this.attachSMF = function()
	{
        tpLastSMFBridge = this;
