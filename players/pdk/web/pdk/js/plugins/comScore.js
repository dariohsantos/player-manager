...
$pdk.plugin.comScore = $pdk.extend(function(){},
{
	...
	initialize:function(lo)
	{
		...
		var testInt = /^\d+$/;
		if(this._lo.vars['minLongformDuration'] && testInt.test(this._lo.vars['minLongformDuration'])) {
			this._minLongformDuration = this._lo.vars['minLongformDuration']*60;
		}
		...
		tpDebug("*** ComScorePlugin plugin LOADED! *** version:[" + this.VERSION + "]");
	...
	{	
		...
		var baseClip = this.getFirstContentBaseClip(playlist.clips);
		var length = -1;
		if(baseClip) {
			length = Math.floor(baseClip.releaseLength / 1000);
		}
		
		this._currentAdState = this.PRE;
		
		if(this.isLongForm(this._minLongformDuration, length, chapterList.chapters))  {
			this._longForm = true;
	...
	getFirstContentBaseClip: function(clips) {
		...
		{
			var clip = clips[i];
			if (!clip.isAd && !clip.noSkip)
				return clip.baseClip;
		}
		// couldn't find any non-ad clip, so just return the first clip
		return clips[0].baseClip;
	},

	isLongForm: function(minLongformDuration, length, chapters) {
		
		if(minLongformDuration != -1 && length != -1) {
	...
	onMediaStart: function(e)
	{
		...
		if (!this._baseClip.isAd)
		{
			...
			if (this._clip.clipIndex === this._chapters - 1)
			{
