...
$pdk.plugin.AdobePass = $pdk.extend(function(){},
{
    ...
    initialize : function(loadObj){
		...
		this.rating = loadObj.vars["defaultRating"];
        ...
        tpDebug("*** adobePass plugin LOADED! ***");
    },
    ...
    getAuthentication: function(){
		...
		if ($pdk.parentUrl)
		{
	        this.getAccessEnabler().getAuthentication($pdk.parentUrl);
		}
		else
		{
    ...
    getAuthorization: function(){
        ...
        if(!this.releaseSet){
			...
			var mrss = '<rss version="2.0" xmlns:media="http://search.yahoo.com/mrss/">' + 
							  	'<channel>' + 
									'<title>' + this.resourceId + '</title>' +
									'<item>' + 
										'<title>' + 'Unknown' + '</title>';

	//									if (_guid)
	//									{
	//											mrss += '<guid>' + {media guid} + '</guid>';
	//									}

										if (this.rating)
										{
											mrss += '<media:rating scheme="urn:v-chip">' + this.rating + '</media:rating>';
										}
            ...
            this.getAccessEnabler().getAuthorization(mrss);
        }
