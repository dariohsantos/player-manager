/************************************************************************
 * Copyright (c) 2009  thePlatform for Media, Inc. All Rights Reserved. *
 ************************************************************************/

package com.theplatform.pdk.plugins.ad
{
	import com.google.ads.instream.api.VideoAdBandwidth;
	import com.theplatform.pdk.controllers.IPlayerController;
	import com.theplatform.pdk.controllers.PlayerController;
	import com.theplatform.pdk.data.Clip;
	import com.theplatform.pdk.data.PrimaryTrack;
	import com.theplatform.pdk.players.NoOpBridge;
	import com.theplatform.pdk.players.PrimaryFlashVideoWrapper;
	import com.theplatform.pdk.utils.Debug;
	
	import flash.events.NetStatusEvent;
	import flash.events.TimerEvent;
	import flash.media.SoundTransform;
	import flash.media.Video;
	import flash.net.NetStream;
	import flash.utils.Timer;
	
	public class NetStreamManager extends NoOpBridge
	{
		private var _controller:PlayerController;
		private var _netStream:NetStream;
		private var _clip:Clip;
		private var _mediaWrapper:PrimaryFlashVideoWrapper;
		private var _durationTimer:Timer;
		private var _loadTimer:Timer;
		private var _sizeVideoTimer:Timer;
		private var _lengthTimer:Timer;
		private var _videoPlayback:IVideoPlayback;
		private var _isMuted:Boolean;
		private var _soundLevel:Number;
		private var _isStream:Boolean = false;
		private var _seeking:Boolean = false;
		
		private var _timeBeforeLastSeek:int = 0;
		
                public function NetStreamManager(controller:IPlayerController, videoPlayback:IVideoPlayback, clip:Clip, video:Video = null)
		{
			super(controller as PlayerController);
			_controller = PlayerController(controller); // ... because we need to access video methods
			_videoPlayback = videoPlayback;
			_clip = clip;
			_netStream = (videoPlayback.ad as IVideoAd).netstream;
			//_netStream.client = this;
			_netStream.addEventListener(NetStatusEvent.NET_STATUS, netStatus);
			
			_isMuted = _controller.getMuteState();
			_soundLevel = _controller.getSoundLevel() / 100;
			var level:Number = _isMuted ? 0 : _soundLevel;
			var st:SoundTransform = new SoundTransform(level);
			_netStream.soundTransform = st;

            setIgnoreClose(true);
			loadStartChecked(true);
			_isStream = _videoPlayback.isNetStreamPlayback();
			if (!_isStream)
			{
				_loadTimer = new Timer(300);
				_loadTimer.addEventListener(TimerEvent.TIMER, loadTick, false, 0, true);
				_loadTimer.start();
			}
			//create a new mediaWrapper
			_mediaWrapper = new PrimaryFlashVideoWrapper(PrimaryTrack.AD, _controller);
			_mediaWrapper.video = video;
			_mediaWrapper.attachNetStream(_netStream);
			_mediaWrapper.setVideoVisible(false);
			_controller.setPrimaryMediaWrapper(_mediaWrapper);
			
			_videoPlayback.play(video);
		}
		
		public function startMediaPlaying():void
		{
			_durationTimer=new Timer(300);
			_durationTimer.addEventListener(TimerEvent.TIMER, sendMediaPlaying);
			_durationTimer.start();
		}
		public function stopMediaPlaying():void
		{
			if (_durationTimer)
			{
				_durationTimer.stop();
				_durationTimer = null; // needed?
			}
		}
		
		
		private function sendMediaPlaying(e:TimerEvent):void
		{
			if (_seeking) return;
			var position:int = 0;
			if (_netStream)
			{
				position = int(1000 * _netStream.time)
			}
			//_controller.trace("media playing:" + position, "InStreamAd.NetStreamManager")
			mediaPlaying(position);
		}
		
		public function cleanUp():void
		{
			_controller.trace("***** netStreamManager.cleanup() ******","InStreamAd.NetStreamManager")
			if (_durationTimer)
			{
				_durationTimer.stop();
				_durationTimer.removeEventListener(TimerEvent.TIMER, sendMediaPlaying);
				_durationTimer = null;
			}
			if (_loadTimer)
			{
				_loadTimer.start();
				_loadTimer.removeEventListener(TimerEvent.TIMER, loadTick);
				_loadTimer = null;
			}
			if (_lengthTimer)
			{
				_lengthTimer.removeEventListener(TimerEvent.TIMER, lengthTick);
				_lengthTimer.stop();
				_lengthTimer = null;
			}
			if (_netStream)
			{
				try
				{
					_netStream.close();
					_netStream.removeEventListener(NetStatusEvent.NET_STATUS, netStatus);
				}
				catch (e:Error) {};
			}
			_mediaWrapper.attachNetStream(null);
		}

		private function netStatus(e:NetStatusEvent):void
		{
			var level:String = e.info["level"];
			var status:String = e.info["code"];
			if (level == "error")
			{
				//some statuses don't deserve an error that stops playback
				if (status == "NetStream.Seek.Failed" || status == "NetStream.Seek.InvalidTime")
				{
					seekFailed();//override in prog flv player
				}
				else
				{
					mediaError();
				}
			}
			else if  (status == "NetStream.Seek.Notify")
			{
				//make sure the timeObject is dispatched
				_seeking = false;
			}
		}
		
		private function loadTick(e:TimerEvent):void
		{
			var percent:Number = _videoPlayback.getPercentLoaded();
			//_controller.trace("instream load: " + percent, "NetStreamManager", Debug.DEBUG)
			var loaded:Number = percent * 100;
			var total:Number = 100;
			mediaLoading(loaded, total);
			
			if (percent == 100)
			{
				_loadTimer.stop();
				_loadTimer.removeEventListener(TimerEvent.TIMER, loadTick);
				_loadTimer = null;
			}
		}
		
			
		////////////////////////
		//	PDK -> Dart
		////////////////////////
		
		public function start():void 
		{ 	
			if (_isStream)
			{
				mediaLoading(100, 100);
			}
			var len:int = _videoPlayback.getVideoLength() * 1000; 
			_controller.trace("setting mediaLength of ad: " + len + " plus one second to allow the media to stop" , "InStreamAd.NetStreamManager", Debug.INFO);
			_clip.mediaLength = len;
			if (len == 15000)
			{
				//huge hack to deal with instream bug.  They're sending in a default value of 15 seconds if they don't actually know the length of the video yet
				//so to deal with their slop, we have to poll the video just in case it's not actually 15 seconds
				_controller.trace("media length is 15000, we can't be sure this is fake data, starting timer", "InStreamAd.NetStreamManager", Debug.INFO);
				startLengthTimer();
			}
			
			_mediaWrapper.sizeVideo();
			

			_mediaWrapper.setVideoVisible(true);
			
			mediaStarts();//superclass
			startMediaPlaying();
		}
		//called from within the plugin--the media has reached its end
		public function end():void
		{
			cleanUp();
			mediaEnds();// superclass. Tell noOp player that video has ended
		}
		public function error():void
		{
			mediaError();
		}
		
		private function startLengthTimer():void
		{
			if (!_lengthTimer)
			{
				_lengthTimer = new Timer(100, 20);//try only for 2 seconds, we don't want to spam the whole video
				_lengthTimer.addEventListener(TimerEvent.TIMER, lengthTick, false, 0, true);
			}
			_lengthTimer.start();
		}
		
		private function lengthTick(e:TimerEvent):void
		{
			var currentLength:Number = _clip.mediaLength;
			var vidLength:Number = _videoPlayback.getVideoLength() * 1000;
			if (vidLength != currentLength)
			{
				_controller.trace("the video changed it's total time to: " + vidLength, "InStreamAd.NetStreamManager", Debug.INFO);
				//the video length updated itself.
				_clip.mediaLength = vidLength;
				_lengthTimer.removeEventListener(TimerEvent.TIMER, lengthTick);
				_lengthTimer.stop();
				_lengthTimer = null;
			}
		}
		
		override public function mediaEndSet():void
		{
			//the pdk has determined that the media must end (perhaps early)
			
			cleanUp();
			mediaEnds(); // Tell noOp player that video has ended
		}
		
		override public function mediaSoundLevelSet(level:Number):void
		{
			_soundLevel = Number(level / 100);
			if (!_isMuted)
			{
				var st:SoundTransform = new SoundTransform(_soundLevel);
				_netStream.soundTransform = st;
			}
		}
		
		override public function mediaMuteSet(isMuted:Boolean):void
		{
			_isMuted = isMuted;
			var transform:SoundTransform = _netStream.soundTransform;
			transform.volume = (_isMuted) ? 0 : _soundLevel;
			_netStream.soundTransform = transform;
		}
		
		override public function mediaSeekSet(position:int):void
		{
			if (!_netStream) return; //ignore
			_timeBeforeLastSeek = int(_netStream.time * 1000);
			_seeking = true;
			try
			{
				var seekPos:Number = Number(position / 1000)
				_netStream.seek(seekPos);
			}
			catch(e:Error)
			{
				seekFailed();
			}
		}
		
		private function seekFailed():void
		{
			if (_netStream) _netStream.seek(_timeBeforeLastSeek/1000);
		}
		
		override public function mediaPauseSet(isPaused:Boolean):void
		{
			//override in subclass
			if (isPaused)
			{
				_netStream.pause();
				stopMediaPlaying(); 
			}
			else
			{
				_netStream.resume();
				startMediaPlaying(); 
			}
			
		}
	}
}
